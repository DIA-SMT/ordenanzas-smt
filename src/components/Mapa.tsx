...
import { BLOQUES, BLOQUE_CORTO, type Concepto } from "@/lib/blocks";
...
/** Mosaico memoizado: no se re-renderiza cuando se hace hover en otro. */
const Tile = memo(function Tile({
  ...
  mounted: boolean;
  pct: number;
  onOpen: (b: number) => void;
  onHover: (b: number, rect: DOMRect) => void;
  onUnhover: () => void;
}) {
  return (
    <div
      className={`tile ${sm ? "sm" : ""} ${dim ? "dim" : ""}`}
    ...
    >
      ...
      <span className="ct">
        ...
        <b><CountUp end={count} /></b> <i>conceptos</i>
      </span>
      {!sm && (
        ...
        <div className="mapa">
          ...
          <div className={`treemap ${isNarrow ? "grid" : ""}`} ref={tmRef}>
            {isNarrow
              ? blocks.map(({ b, rows, dom }, i) => (
                  <div className="tm-grid-cell" key={b}>
                    <Tile
                      ...
                      dim={isDim(dom)}
                      mounted={mounted}
                      pct={Math.round((rows.length / stats.maxBloque) * 100)}
                      onOpen={onOpenBloque}
                      onHover={onHover}
                      onUnhover={onUnhover}
                    />
              ...
              : rects.map((r, i) => {
                  const sm = (r.w * r.h) / totalArea < 0.05 || r.item.value <= 2;
                  return (
                    <div
                      className="tm-cell"
                      key={r.item.b}
                    ...
                    >
                      <Tile
                        ...
                        i={i}
                        sm={sm}
                        dim={isDim(r.item.dom)}
                        mounted={mounted}
                        pct={Math.round((r.item.value / stats.maxBloque) * 100)}
                        onOpen={onOpenBloque}
                        onHover={onHover}
                        onUnhover={onUnhover}
                      />
